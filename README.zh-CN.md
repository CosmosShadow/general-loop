...
General Loop 不限定执行角色。产品、开发、测试、发布、运营和客服 Agent 都可以围绕同一组 Task 协作。配合神念 AI 群聊时，客户反馈可以先由客服 Agent 澄清并写成 Task，再由 Loop 安排调查、修复、测试与发布，最后回到客服完成回告：

```text
客户反馈 → 客服 Agent 澄清并写 Task → Loop 调度
        → 调查 / 开发 / 测试 / 发布 Agent → 客服回告
```

因此它既能协调内部交付，也能把外部客户入口接入同一个持续运行的工作闭环。真人仍然负责目标、风险和授权；日常分工、状态和证据由 Agent 团队持续维护。
